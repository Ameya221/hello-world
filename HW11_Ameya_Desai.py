...
class Repository:
    ...
    def __init__(self, path1, path2, path3, path4):
        ...
        self.instructor_check = defaultdict(list)
    ...
    def Student(self):
        try:
            ...
            for student_CWID, course, grade, instructor_cwid in file_reading_gen(self.path_grades, 4, sep="\t", header=True):
                if student_CWID not in self.student_name.keys():
                    print("Error: Unknown student in grades file")
                    exit()
            ...
            for k, v in self.majors_summery_dict.items():
                for key, val in self.student_name_dept_courses.items():
                    if val[1] not in self.majors_summery_dict.keys():
                        print("Error: Student whose id is ",key, " has unknown major")
                        exit()
                    if k == val[1]:
            ...
            for CWID, instructor, dept in file_reading_gen(self.path_instructors, 3, sep="\t", header=True):
                self.instructor_check[CWID].append(instructor)
    ...
    def Instructor(self):
        ...
        try:
            for studentCWID, course, grade, instructorCWID in file_reading_gen(self.path_grades, 4, sep="\t", header=True):
                if instructorCWID not in self.instructor_check.keys():
                    print("unknown instructor with id ", instructorCWID, " in grades file")
                    exit()
